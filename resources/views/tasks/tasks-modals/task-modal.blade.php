    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <form id="add-new-task-form" action="{{route('tasks.store', $project_id)}}" method="post"
                      class="needs-validation" novalidate>
                    @csrf
                    <div class="row">
                        <div class="col-12">
                            <input name="tasks_list_id" type="hidden"
                                   value="{{isset($task_list->id) ? $task_list->id : ''}}">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="task-description" class="form-control" rows="5"
                                          placeholder="{{__('task.description')}}"></textarea>
                            </div>
                    ...
                    <form id="edit-task-form" action="" method="post" class="needs-validation" novalidate>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    <label class="control-label">{{__('task.title')}} *</label>
                                    <input type="text" id="edit-task-title" name="title" class="form-control"
                                           placeholder="{{__('task.title')}}" required>
                                    <div class="invalid-feedback">
                                        {{__('task.task_title_required')}}
                                    </div>
                                ...
                                <div class="form-group">
                                    <label class="control-label">{{__('task.description')}}</label>
                                    <textarea id="edit-task-description" class="form-control" rows="5"
                                              placeholder="{{__('task.description')}}"></textarea>
                                </div>

                                <div class="form-group">
                                    <label for="employee" id="edit-select-employee">{{ __('task.employee') }} *</label>
                                    <select class="form-control select2-multiple" data-toggle="select2"
                                            multiple="multiple" data-placeholder="" required>
                                        @foreach($employees as $employee)
                                            <option
                                                value="{{$employee->id}}">{{$employee->first_name.' '.$employee->last_name}}</option>
                                        @endforeach
                        ...
                        <div class="row mt-2">
                            <div class="col-3">
                                <button type="button" class="btn btn-danger btn-delete-task" url="{{route('tasks.destroy', [$project->id, $task->id])}}">{{__('tasks_list.delete')}}</button>
                            </div>
                            <div class="col-9 text-right">
                                <button type="button" class="btn btn-light mr-1" data-dismiss="modal">{{__('task.cancel')}}</button>
                                <button type="submit" class="btn btn-success btn-edit-task" data="{{$task->title.'$$'.$task->description.'$$'.$task->employee_id.'$$'.route('tasks.update', [$project->id, $task->id])}}">{{__('task.edit')}}</button>
                            </div>
                        </div>
                    </form>
                </div>
